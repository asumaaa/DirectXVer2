...
ID3D12Device* ChromaticAberration::device = nullptr;
ComPtr<ID3D12RootSignature>ChromaticAberration::rootsignature;
ComPtr<ID3D12PipelineState>ChromaticAberration::pipelinestate;
const float ChromaticAberration::clearColor[4] = { 0.25f,0.5f, 0.1f, 0.0f };

void ChromaticAberration::Initialize()
{
	HRESULT result;
...
void ChromaticAberration::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void ChromaticAberration::CreateGraphicsPipeLine()
{
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/ChromaticAberration/ChromaticAberrationVertexShader.hlsl",
		nullptr,
	...
	//ピクセルシェーダの読み込み コンパイル
	result = D3DCompileFromFile(
		L"Resources/Shaders/ChromaticAberration/ChromaticAberrationPixelShader.hlsl",
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main", "ps_5_0",
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION,
...
void ChromaticAberration::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
...
void ChromaticAberration::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
	CD3DX12_RESOURCE_BARRIER a0 = CD3DX12_RESOURCE_BARRIER::Transition(
		textureBuff.Get(),
