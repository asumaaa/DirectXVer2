...
ID3D12Device* ReversalEffect::device = nullptr;
ComPtr<ID3D12RootSignature>ReversalEffect::rootsignature;
ComPtr<ID3D12PipelineState>ReversalEffect::pipelinestate;
const float ReversalEffect::clearColor[4] = { 0.25f,0.5f, 0.1f, 0.0f };


void ReversalEffect::Initialize()
{
	HRESULT result;
...
void ReversalEffect::Draw(ID3D12GraphicsCommandList* cmdList)
{
	//パイプライン、ルートシグネチャをセット
...
void ReversalEffect::CreateGraphicsPipeLine()
{
	HRESULT result;
...
void ReversalEffect::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
...
void ReversalEffect::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
	CD3DX12_RESOURCE_BARRIER a0 = CD3DX12_RESOURCE_BARRIER::Transition(
		textureBuff.Get(),
