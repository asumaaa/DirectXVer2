...
ComPtr<ID3D12RootSignature>FbxObject3D2::rootsignature;
ComPtr<ID3D12PipelineState>FbxObject3D2::pipelinestate;

ID3D12Device* FbxObject3D2::device = nullptr;
Camera* FbxObject3D2::camera = nullptr;
LightGroup* FbxObject3D2::lightGroup = nullptr;

void FbxObject3D2::Initialize()
{
	//1フレーム分の時間を60FPSで設定
...
void FbxObject3D2::Draw(ID3D12GraphicsCommandList* cmdList)
{
	//モデルの割り当てがなければ描画市内
	if (model == nullptr)
	{
...
void FbxObject3D2::CreateGraphicsPipeline()
{
