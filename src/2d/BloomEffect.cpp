...
ID3D12Device* BloomEffect::device = nullptr;
ComPtr<ID3D12RootSignature>BloomEffect::rootsignature;
ComPtr<ID3D12PipelineState>BloomEffect::pipelinestate;
const float BloomEffect::clearColor[4] = { 0.25f,0.5f, 0.1f, 0.0f };
...
void BloomEffect::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void BloomEffect::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
...
void BloomEffect::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	//リソースバリアを変更
	CD3DX12_RESOURCE_BARRIER a0 = CD3DX12_RESOURCE_BARRIER::Transition(
		textureBuff.Get(),
